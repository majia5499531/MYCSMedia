...
-(void)setVideoCellData:(ContentModel*)objc albumnName:(NSString *)albumnName simpleMode:(BOOL)simple
{
    ...
    [videoCoverImage sd_setImageWithURL:[NSURL URLWithString:dataModel.imagesUrl] placeholderImage:nil];
...
{
    //如果不是当前数据，return
    if ([dataModel.id isEqualToString:currentId]==NO)
    {
        return;
    }
    
    //当前NavigationController并非TabbarController焦点，return
    UINavigationController * nav = [self getCurrentNavigationController];
    UITabBarController * tabvc = nav.tabBarController;
    if ([tabvc.selectedViewController isEqual:nav]==NO)
    {
        return;
    }
    
    //如果是在SZVideoListVC容器中
    UIViewController * fathervc = [self getCurrentViewController];
    if([fathervc isKindOfClass:[SZVideoListVC class]])
    {
        //当前容器并非焦点，return
        SZVideoListVC * currentvc = (SZVideoListVC*)fathervc;
        if (currentvc.isFocusedVC==NO)
        {
            return;
        }
        
        UIViewController * containerVC = [currentvc.view.superview getCurrentViewController];
        if ([nav.topViewController isEqual:containerVC]==NO)
        {
            return;
        }

//        NSLog(@"CellNeedPlay_%@_%@_%@_%@_%@",currentId,currentvc,containerVC,nav,nav.tabBarController);
        [self playingVideo];
    }
    
    
    //如果是在详情里
    else if ([fathervc isKindOfClass:[SZVideoDetailVC class]])
    {
        
        if ([nav.topViewController isEqual:fathervc]==NO)
        {
            return;
        }
        
//        NSLog(@"CellNeedPlay_%@_%@_%@_%@_%@",currentId,fathervc,fathervc,nav,nav.tabBarController);
        [self playingVideo];
    }
...
-(void)playingVideo
{
    ...
    [MJVideoManager playWindowVideoAtView:videoCoverImage url:dataModel.playUrl contentModel:dataModel renderModel:renderMode controlMode:MJCONTROL_STYLE_SHORT_VIDEO];
